...
```
Получение данных по внешним ключам таблиц (отсеиваем несценарные таблицы, т.к. в них мы на автомате ничего не меняем).
```
CREATE VIEW utils.scenarioTableForeignKeys AS
...
where tco.constraint_type = 'FOREIGN KEY' AND kcu.table_schema = 'data' AND rel_tco.table_name IN (SELECT tableName FROM utils.scenarioTables);
```
### Организация работы со слепками с помощью процедур
Процедура создания слепка
```
CREATE PROCEDURE utils.addScenario(name VARCHAR(255), description TEXT, createdByUsername VARCHAR(255), createdScenarioId OUT INT, sourceScenarioId INT DEFAULT NULL) AS $$
...
DECLARE fkRow utils.scenarioTableForeignKeys%rowtype;
BEGIN
  INSERT INTO utils.scenarios (name, description, createdByUsername) VALUES (name, description, createdByUsername) RETURNING id INTO insertedId;
  ...
  IF sourceScenarioId IS NOT NULL THEN
    ...
    WHILE EXISTS (SELECT 1 FROM utils.scenarioTables WHERE tableName NOT IN (SELECT tableName FROM copiedTables)) LOOP
      SELECT tableName INTO currentTableName FROM utils.scenarioTables WHERE NOT EXISTS (SELECT 1 FROM utils.scenarioTableForeignKeys WHERE fkTable = tableName AND pkTable NOT IN (SELECT tableName FROM copiedTables)) AND tableName NOT IN (SELECT tableName FROM copiedTables);
